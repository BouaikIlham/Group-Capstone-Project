...
const fetchMovieReserve = async (movieId) => {
...
const getTotalComments = async (movieId) => {
  const res = await fetchMovieReserve(movieId)
...
document.addEventListener('click', async (e) => {
  if (e.target.matches('.comment-btn')) {
    const data = await fetchMovieReserve(e.target.id);
...
export { fetchMovieReserve, commentCounter };
